...
function Blueprint.parse(bp_data, chain_signal, is_diagonal, ents)
    ...
    box.left_top.x = math.floor(box.left_top.x - 0.75) - 1
    box.right_bottom.x = math.ceil(box.right_bottom.x + 0.75) + 1
    ...
    if not is_diagonal then
        ...
        box.left_top.y = -1--box.left_top.y + 0.5
        box.right_bottom.y = 0
    end
    box.h = math.abs(box.right_bottom.y - box.left_top.y)

    bp_data.bounding_box = box
    bp_data.bb_l = {left_top = {x = box.left_top.x, y = box.left_top.y}, right_bottom = {x = 0, y = box.right_bottom.y}}
    bp_data.bb_r = {left_top = {x = 0, y = box.left_top.y}, right_bottom = {x = box.right_bottom.x, y = box.right_bottom.y} }
