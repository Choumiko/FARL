...
SED_FILES := $(shell find . -iname '*.json' -type f -not -path "./build/*") $(shell find . -iname '*.lua' -type f -not -path "./build/*") $(shell find . -iname 'changelog.txt' -type f -not -path "./build/*") $(shell find . -iname '*.png' -type f -not -path "./build/*")
...
verify:
	luacheck . --exclude-files scrap.lua profiler3.lua unused.lua
